...
function transform(trans::DNTransformation, g::DecisionNetwork)
    transform(trans, graph(g))(; implementation(g)...)
end

function transform(trans::Insert, g::DecisionGraph)
    new_nodes = merge(nodes(g), NamedTuple([name(output(n)) => n for n in trans.nodes]))
