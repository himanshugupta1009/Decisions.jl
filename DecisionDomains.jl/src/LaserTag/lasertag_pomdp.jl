...
struct LaserTagPOMDP{S} <: POMDPs.POMDP{LTState, Symbol, SVector{5,Int}}
    size::SVector{2, Int}
...
function POMDPs.initialstate(m::LaserTagPOMDP)
    ...
    return POMDPTools.Uniform(states)
end
...
#=
l = DiscreteLaserTagPOMDP()
b0 = POMDPs.initialstate(l)
s = rand(b0)
a = POMDPs.actions(l)[5]
sp = rand(POMDPs.transition(l,s,a))
o = rand(POMDPs.observation(l,a,sp))
r = POMDPs.reward(l,s,a,sp)
println(s,a,sp,o,r)

Distributions.support(POMDPs.transition(l,s,a))
