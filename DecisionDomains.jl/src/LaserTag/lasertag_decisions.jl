...
function LaserTagDecisionsPOMDP(; pomdp = DiscreteLaserTagPOMDP())
    ...
    lt_initial = @ConditionalDist @NamedTuple{s::LTState, m::typeof(m0)} begin
        ...
        support(; ) = (; s = POMDPTools.ordered_states(pomdp), m = m0)
        #Q: Or support(; )   = [ (; s = s, m = m0) for s in POMDPTools.ordered_states(pomdp) ]
    end
...
function DecisionProblems.solve(::RandomSolver, dp::DecisionProblem)
    ...
    pol = @ConditionalDist Symbol begin
        ...
        #Q: Which support functtion essentially gets called here?
        #Q: Why does this also work?
        # rand(rng; m) = rand(rng, DecisionNetworks.support(dp[:a];m=m))   
...
1. What is the expected return type for initial when DN is a POMDP? Tuple? NamedTuple? Is it just a belief or both state and belief?  
2. What is support(;) supposed to return exactly in memory for a POMDP? - A belief distribution or the full set of states?
3. Issues with Ben's RockSample
